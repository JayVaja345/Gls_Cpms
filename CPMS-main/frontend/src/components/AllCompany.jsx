 ...
 const fetchCompanys = async () => {
  try {
    ...
    if (error.response?.status === 403) {
      ...
      setError("You don't have permission to view company data");
    } else {
  ...
  const confirmDelete = async (companyId) => {
    try {
      ...
      setShowModal(false);
      if (error?.response?.status === 403) {
        setToastMessage("You don't have permission to delete companies");
